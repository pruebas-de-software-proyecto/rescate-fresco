...
vi.mock('../../components/NavBar', () => ({
    default: () => _jsx("div", { "data-testid": "navbar", children: "NavBar" }),
}));
...
describe('ReservationsPage', () => {
    ...
    it('muestra estado de carga durante la cancelación', async () => {
        ...
        vi.mocked(FullLotesAPI.getAll).mockResolvedValue(mockAllProducts);
        let resolveUpdate;
        vi.mocked(FullLotesAPI.update).mockImplementation(() => new Promise(resolve => { resolveUpdate = resolve; }));
        render(_jsx(ReservationsPage, {}));
        await waitFor(() => {
            expect(screen.getByText('Manzanas Rojas Premium')).toBeInTheDocument();
        });
    ...
    it('muestra mensaje de confirmación con el nombre del producto', async () => {
        ...
        const cancelButtons = screen.getAllByText('Cancelar Reserva');
        await user.click(cancelButtons[0]);
        expect(window.confirm).toHaveBeenCalledWith('¿Estás seguro de que quieres cancelar la reserva de "Manzanas Rojas Premium"?');
    });
    ...
    it('maneja productos sin fotos usando imagen por defecto', async () => {
        ...
        await waitFor(() => {
            ...
            const productImage = images.find(img => img.getAttribute('src')?.includes('bupasalud.com'));
